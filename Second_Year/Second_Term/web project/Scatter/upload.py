...
if __name__ == "__main__":
    while True:
        data_row_id = input("Enter the UUID (example: 475a1575-6132-4744-a8e2-3e36dbcf8bf1): ").strip()
        # Accept both UUID format and the old cm format
        if (len(data_row_id) == 36 and data_row_id.count('-') == 4) or (data_row_id.startswith("cm") and data_row_id.isalnum()):
            break
        print("Invalid ID. Please enter a valid UUID format (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx) or Data Row ID starting with 'cm'.")
